...
public class WaveformSnapshotViewer {
	...
	private void hookDragAndDrop(final Composite parent) {
		...
		new ControlSystemDropTarget(parent, ChannelInfo[].class,
				ProcessVariable[].class, ArchiveDataSource[].class, File.class,
				String.class) {
			@Override
			public void handleDrop(final Object item) {
				if (item instanceof ChannelInfo[]) {
					...
					for (ChannelInfo channel : channels)
						droppedPVName(channel.getProcessVariable(),
					...
					try {
						...
						ExceptionDetailsErrorDialog.openError(
								parent.getShell(), Messages.Error, ex);
					}
				}
			}
		};
     ...
     */
	private void droppedPVName(final ProcessVariable name,
		...
		if (name == null) {
			...
			for (ModelItem item : plotModel.getItems()) {
				...
				if (pv.hasArchiveDataSource(archive))
					continue;
				new AddArchiveCommand(plot.getUndoableActionManager(), pv,
						archive);
			}
		} else { // Received PV name
					// Add the given PV to the model anyway even if the same PV
					// exists in the model.
			final UndoableActionManager operations_manager = plot
					.getUndoableActionManager();

			// Add to first empty axis, or create new axis
			final AxisConfig axis = plotModel.getEmptyAxis().orElseGet(
					() -> new AddAxisCommand(operations_manager, plotModel)
							.getAxis());
			// Add new PV
			AddModelItemCommand.forPV(shell, operations_manager, plotModel,
					name.getName(), Preferences.getScanPeriod(), axis, archive);
			return;
		}
	}
	...
	private void droppedName(final String name) {
		...
		final AddPVAction add = new AddPVAction(
				plot.getUndoableActionManager(), shell, plotModel, false);
	...
	private void droppedFilename(String file_name) {
		...
		final String url = ImportArchiveReaderFactory
				.createURL(type, file_name);
		final ArchiveDataSource imported = new ArchiveDataSource(url, 1, type);
		// Add PV Item with data to model
		AddModelItemCommand.forPV(shell, operations_manager, plotModel,
				dlg.getItemName(), Preferences.getScanPeriod(), axis, imported);
	}
	...
	/**
	 * Method which creates retrieval job for model item. When data are
	 * retrieved, data provider with waveform values is created and shown on
	 * plot.
	 * 
	...
	private void retrieveSample(final ModelItem item, Instant timestamp) {
		...
		ArchiveFetchJob fetchJob = new ArchiveFetchJob(pvItem, timestamp,
				timestamp, new ArchiveFetchJobListener() {
